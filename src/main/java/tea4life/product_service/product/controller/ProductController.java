package tea4life.product_service.product.controller;
...
import tea4life.product_service.category.service.ProductCategoryAdminService;
...
public class ProductController {
    ...
    public ApiResponse<List<ProductSummaryResponse>> getRelatedProducts(
            @PathVariable("id") Long id,
            @RequestParam(value = "limit", required = false, defaultValue = "10") Integer limit
    ) {
        return new ApiResponse<>(productService.getRelatedProducts(id, limit));
    }

    @GetMapping("/{id}/option-values/recommended")
    public ApiResponse<List<RecommendedOptionValueResponse>> getRecommendedOptionValues(
            @PathVariable("id") Long id,
            @RequestParam(value = "limit", required = false, defaultValue = "10") Integer limit
    ) {
        return new ApiResponse<>(productService.getRecommendedOptionValues(id, limit));
    }
